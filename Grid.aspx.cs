...
public partial class Grid : System.Web.UI.Page
{
  ...
  public DataTable GetSellers()
  {
    ...
    */
    ...
    DataTable henkilot = DAL.HaeHenkilot();
    henkilot.Columns.Add("IKA", typeof(int)).DefaultValue = 0;
    ...
    henkilot.PrimaryKey = new DataColumn[] { henkilot.Columns[0] };       //new DataColumn[] { data.Columns["ID"] };
  ...
  {
    RadGrid1.DataSource = GetSellers();
  }
  ...
  protected void RadGrid1_UpdateCommand(object sender, Telerik.Web.UI.GridCommandEventArgs e)
  {
    ...
    DataRow row = this.Sellers.Rows.Find((e.Item as GridEditableItem).GetDataKeyValue("HENKILO_ID"));

    foreach (string key in table.Keys)
    {
      if (key == "KAUPUNKI_NIMI")
        row["KAUPUNKI_ID"] = table[key];
      else
    ...
    */
    ...
    DAL.PaivitaHenkilo((int)row["HENKILO_ID"], row["NIMI"].ToString(), Int32.Parse(row["ARVIO"].ToString()), (DateTime)row["SYNTYMAAIKA"], Int32.Parse(row["KAUPUNKI_ID"].ToString()));
  ...
  protected void RadGrid1_InsertCommand(object sender, GridCommandEventArgs e)
  {
    ...
    row["HENKILO_ID"] = new Random().Next(int.MaxValue);
    DateTime date;
    if (DateTime.TryParse((row["SYNTYMAAIKA"].ToString()), out date))
    {
      row["IKA"] = DateTime.Now.Date.Year - date.Year;
    }
    this.Sellers.Rows.InsertAt(row, 0);
    DAL.LisaaHenkilo(row["NIMI"].ToString(), Int32.Parse(row["ARVIO"].ToString()), (DateTime)row["SYNTYMAAIKA"], Int32.Parse(row["KAUPUNKI_NIMI"].ToString()));
  }

  protected void RadGrid1_DeleteCommand(object sender, GridCommandEventArgs e)
  {
    DAL.PoistaHenkilo(Int32.Parse(this.Sellers.Rows.Find((e.Item as GridEditableItem).GetDataKeyValue("HENKILO_ID"))["HENKILO_ID"].ToString()));
    this.Sellers.Rows.Remove(this.Sellers.Rows.Find((e.Item as GridEditableItem).GetDataKeyValue("HENKILO_ID")));
