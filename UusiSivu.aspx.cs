...
{
  TarkistusSivuDAL DAL = new TarkistusSivuDAL();
  ...
  public DataTable GetProjects()
  {
    ...
    */
    ...
    DataTable datankeruu = DAL.HaeProsessit();
    datankeruu.Columns.Add("PROJEKTIN_NIMI", typeof(char)).DefaultValue = 0;

    /* Lasketaan iät syntymäajan perusteella:
    foreach (DataRow rv in datankeruu.Rows)
    {
      rv["IKA"] = DateTime.Now.Year - ((DateTime)rv["SYNTYMAAIKA"]).Year;
    }*/
    //Asetetaan primary key alempana käytettyjä find-komentoja varten:
    datankeruu.PrimaryKey = new DataColumn[] { datankeruu.Columns[0] };       //new DataColumn[] { data.Columns["ID"] };

    return datankeruu;
    }

  /*public DataTable GetCities() voi heittää tän kommentteihi ku käytetään toistaseks vaan yhtä taulua.
  {
  ...
  protected void RadGrid1_UpdateCommand(object sender, Telerik.Web.UI.GridCommandEventArgs e)
  {
    ...
    DataRow row = this.Projects.Rows.Find((e.Item as GridEditableItem).GetDataKeyValue("ID"));

    foreach (string key in table.Keys)
    {
      if (key == "PROSESSIN_NIMI")
        row["ID"] = table[key];
      else
...
DAL.PaivitaRivi((int)row["ID"], row["PROSESSIN_NIMI"].ToString(), row["MITATTAVA_ASIA"].ToString(), row["DATATYYPPI"].ToString(), row["TOIM_MAARITELMA"].ToString(), row["KESK_TEKIJAT"].ToString(),
    row["KUINKAPALJONTIETOAKERATAAN"].ToString(), row["VASTUUHENKILOT"].ToString());
  ...
  protected void RadGrid1_InsertCommand(object sender, GridCommandEventArgs e)
  {
    ...
    this.Projects.Rows.InsertAt(row, 0);
        DAL.LisaaRivi((int)row["ID"], row["PROSESSIN_NIMI"].ToString(), row["MITATTAVA_ASIA"].ToString(), row["DATATYYPPI"].ToString(), row["TOIM_MAARITELMA"].ToString(), row["KESK_TEKIJAT"].ToString(),
            row["KUINKAPALJONTIETOAKERATAAN"].ToString(), row["VASTUUHENKILOT"].ToString());
    } 
 
  protected void RadGrid1_DeleteCommand(object sender, GridCommandEventArgs e)
  {
    DAL.PoistaRivi(Int32.Parse(this.Projects.Rows.Find((e.Item as GridEditableItem).GetDataKeyValue("ID"))["ID"].ToString()));
